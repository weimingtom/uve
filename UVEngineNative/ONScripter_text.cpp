...
bool ONScripter::clickNewPage( char *out_text )
{
    ...
    flush( REFRESH_NONE_MODE/*,(SDL_Rect*)0,true,false,true */);
    skip_mode &= ~SKIP_TO_EOL;
...
int ONScripter::textCommand()
{
	...
	//IsProcessing=true;
    while(processText());
	//refreshSurface(accumulation_surface, &dirty_rect.bounding_box/*&rect*/, REFRESH_NORMAL_MODE);
	//IsProcessing=false;
    return RET_CONTINUE;
}
...
void ONScripter::processEOT()
{
    if ( skip_mode & SKIP_TO_EOL ){
        flush( refreshMode(),(SDL_Rect* )0,true,false,true );
        skip_mode &= ~SKIP_TO_EOL;
    }
