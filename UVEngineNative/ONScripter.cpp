...
#define DEFAULT_ENV_FONT "ENV_FONT"
...
void ONScripter::initSDL(D3DSharedData* sdata)
{
    /* ---------------------------------------- */
    /* Initialize SDL */

	if ( SDL_Init( SDL_INIT_VIDEO | SDL_INIT_TIMER | SDL_INIT_AUDIO,sdata ) < 0 ){
	...
	window = SDL_CreateWindow(NULL, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, hardwareW, hardwareH, 0);
	if (window == nullptr){
...
#endif        
		CALL(ErrorLog(L"Couldn't open audio device!\n"));
...
ONScripter::ONScripter()
{
    ...
    midi_cmd  = NULL;
#else
    midi_cmd  = NULL;/*getenv("MUSIC_CMD");*/
#endif
...
int ONScripter::init(D3DSharedData *sdata)
{
    initSDL(sdata);
    openAudio();
    ...
    if ( sentence_font.openFont( font_file, screen_ratio1, screen_ratio2 ) == NULL ){
		CALL(ErrorLog(L"can't open font file \n"));
...
void ONScripter::flush( int refresh_mode, SDL_Rect *rect, bool clear_dirty_flag, bool direct_flag, bool click )
{
    if ( direct_flag ){
        ...
        if (dirty_rect.bounding_box.w * dirty_rect.bounding_box.h > 0)
            flushDirect( dirty_rect.bounding_box, refresh_mode/*, click */);
    }
    
    if ( clear_dirty_flag ) dirty_rect.clear();
}
//bool IsProcessing=false;
void ONScripter::SetScreenResolution(int w,int h)
{
	this->hardwareW=w;
	this->hardwareH=h;
}
void ONScripter::flushDirect( SDL_Rect &rect, int refresh_mode, bool click)
{
	//char a[100];
	...
	SDL_UpdateTexture(texture, NULL, accumulation_surface->pixels, accumulation_surface->pitch);
	//if (&rect != nullptr)
	//{
	//	rect.x = ((float )rect.x) /1.25f;
	//	rect.y = ((float)rect.x) /1.25f;
	//	rect.w = ((float)rect.x) /1.25f;
	//	rect.h = ((float)rect.x) /1.25f;
	//}
	//try{
	...
	//	SDL_UpdateTexture(texture, &rect, accumulation_surface->pixels, accumulation_surface->pitch);
	...
	SDL_RenderPresent((SDL_Renderer*)&screen_surface);
    //SDL_UpdateRect( screen_surface, dst_rect.x, dst_rect.y, dst_rect.w, dst_rect.h );
#endif
}
void ONScripter::SetDisplayMode(UINT flag)
	/*
	0xF for normal 4:3 games
	0xFF for wide-screen games;
	0xFFF for forced wide screen 
	NOTICE : Invalid parameter will cause an Exception
	*/
{
	switch (flag)
	{
...
void ONScripter::mouseOverCheck( int x, int y )
{
    ...
    while( bl ){
		//sprintf(b,"%d\t %d\t %d\t %d",bl->select_rect.x,bl->select_rect.y,bl->select_rect.w,bl->select_rect.h);
