...
int SDL_main(ONScripter* ons,void *driverdata);
#endif
...
private:
	...
	void initSDL(D3DSharedData* sdata);
    void openAudio();
	public:
    ...
    void flush( int refresh_mode, SDL_Rect *rect=NULL, bool clear_dirty_flag=true, bool direct_flag=false, bool click=false );
    void flushDirect( SDL_Rect &rect, int refresh_mode, bool click=false );
...
private:
    ...
    // format = SDL_PIXELFORMAT_RGB565 for any 16bit surface without SDL_Renderer (Android, Zaurus)
		Uint32 texture_format;//MODIFIED
