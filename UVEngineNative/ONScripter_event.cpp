...
bool ONScripter::mousePressEvent( SDL_MouseButtonEvent *event )
{
	...
	//char a[20];
	//char b[20];
	//sprintf(a,"%d \t %d ",event->x,event->y);

    current_button_state.x = event->x /** screen_width / screen_device_width*/;
    current_button_state.y = event->y /** screen_width / screen_device_width*/;
	//sprintf(b,"%d \t %d",event->x,event->y);
		...
		case SDL_FINGERUP:
          ...
          case SDL_FINGERMOTION:
            {
				...
				tmp_event.motion.x = device_width *event.tfinger.x/touch->fingers[0]->x/*->xres*/ - (device_width -screen_device_width)/2;
                tmp_event.motion.y = device_height*event.tfinger.y/touch->fingers[0]->y/*yres*/ - (device_height-screen_device_height)/2;
                if (mouseMoveEvent( &tmp_event.motion )) return;
                if (btndown_flag){
                    event.button.type = SDL_MOUSEBUTTONDOWN;
                    event.button.button = SDL_BUTTON_LEFT;
          ...
          case SDL_FINGERDOWN:
          {
            ...
            if ( btndown_flag ){
                ...
                if (touch->num_fingers >= 2)
                    tmp_event.button.button = SDL_BUTTON_RIGHT;
                tmp_event.button.x = device_width *event.tfinger.x/touch->fingers[0]->x/*->xres*/ - (device_width -screen_device_width)/2;
                tmp_event.button.y = device_height*event.tfinger.y/touch->fingers[0]->y/*yres*/ - (device_height-screen_device_height)/2;
                ret = mousePressEvent( &tmp_event.button );
            }
            {
                SDL_Touch *touch = SDL_GetTouch(event.tfinger.touchId);
