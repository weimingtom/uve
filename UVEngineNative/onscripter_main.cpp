...
#elif defined(QWS) || defined(ANDROID)
...
#endif
{
...
#if defined(WINDOWS_PHONE_8)
	ons->setArchivePath(GAMEPATH);
	string s=string(GAMEPATH);
    ...
    if (ons->openScript())CALL(ExitCore(-1)); //exit(-1);
	if (ons->init((D3DSharedData *)driverdata)) CALL(ExitCore(-1));//exit(-1);
    ons->executeLabel();
    CALL(ExitCore(0));
    //exit(0);
	return 0;
